...
import {COIN_PRICE} from '../utils/globals';
const debug = Debug('dbFunctions:');
...
export async function surroundingUsers(league, userId, limit) {
...
export function getRecords(league, limit, page) {
...
export async function userRank(league, userId) {
...
export async function exchangecouponsToLeagueOppo(userId, oppo, league) {  // transaction
   // let[nr,user]=null;
...
export async function exchangecoinsToMoney(userId, coins) {
    const session = await mongoose.startSession();
    session.startTransaction();
    try {
        const opts = {session};
...
export async function giveRewards(collectionName) {
    const session = await mongoose.startSession();
    ...
    try {
        ...
        if(league.loyaltyGivens && league.loyaltyGivens !== 0) {
            const loyaltiesUsers : any = await getRecords(league.collectionName,league.loyaltyGivens,1);
...
export function saveBoxTransaction(userId, transactionId, boxId) {
    const p = {
        user: userId,
        transactionId,
        box: boxId
...
export async function buyBox(userId, transactionId, boxId) {
