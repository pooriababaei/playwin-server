...
import {ADVERTISE_AWARD_COUPON,NEWEST_APP_VERSION} from '../utils/globals';
const User = mongoose.model('user');
const debug = Debug('User Controller:');

export function checkNewVersion(req, res) {
    if (req.params.version < NEWEST_APP_VERSION) { // there is a new version
...
export async function getUsers(req, res) {
    ...
    if (filter && filter.phoneNumber) { query.find({
        phoneNumber: {
...
export function updateUser(req, res) { // need to be checked
    ...
    User.findOneAndUpdate({
        ...
        ...info,
        $inc: {
            coupons: extraCoupons === 'true' ? ADVERTISE_AWARD_COUPON : 0
        }
    }, {
        new: true
    }, (err, user) => {
