...
export function createGame(req, res) {
    ...
    if (req.files && req.files.game) {
        const zip = new AdmZip(path.join(__dirname, '../../public/games/',
            req.body.name, req.files.game[0].originalname));

        zip.extractAllTo(path.join(__dirname, '../../public/games/', req.body.name), /*overwrite*/ true);
    ...
    if (req.files && req.files.images) {
        for (let i = 0; i < req.files.images.length; i++) {
            const temp = '/public/games/' + req.body.name + '/images/' + req.files.images[i].filename;
            images.push(temp);
        }
        info.images = images;
    }
