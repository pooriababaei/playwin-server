...
import {sendSMS} from '../utils/functions';
...
export async function signup (req,res) {
    ...
    if (req.body.invitedBy && req.body.invitedBy !== info.username && req.body.invitedBy.length > 0) {
        const inviter = await User.findOne({username: req.body.invitedBy}).lean().catch(() => res.sendStatus(500));
        if(inviter) {
            info.coupons = INVITE_REWARD;
        }
    }
